...
void Polygon::validate() {
	try {
		try {
			for (const auto& v : theVerts) {
				if (v.x() < 0 || v.y() < 0)
					throw out_of_range("polygon vert less zero");
			}
		...
		if (this->concave()) {
			throw domain_error("polygon concave");
		}
	} catch (const domain_error& p) {
		std::cout << p.what() << std::endl;
		vec2 coordinate = theVerts[theVerts.size()-1];
		this->theVerts.resize(3);
		this->theVerts[2] = coordinate;
